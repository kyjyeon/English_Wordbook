...
public:
	...
	*/
	...
	/**
	*	@brief	Move the current position index to next index to get next data
	*/
	int GetNext(T& data);

	/**
	*	@brief	Reset the current position integer to default setting 
	*/
	int ResetList();

	/**
	*	@brief	Search for data inside the array matching with parameter data
	*/
	int Get(T& data);

	/**
	*	@brief	Insert the parameter data into array.
	*/
	int Insert(T& data);

	/**
	*	@brief	Delete data inside array matching with parameter instance.
	*/
	int Delete(T& data);

	/**
	*	@brief	Return the address of current list index
	T* GetCurPtr();
	...
	*/
	...
	/*
	*	@brief	return true if the list is empty, false if the list isn't empty. size, false if smaller than maximum size
	*/
	bool isEmpty();

	/*
	*	@brief	return true if current list size matches the maximum size, false if smaller than maximum size
	*/
	bool isFull();

	/*
	*	@brief	Empty the current list by deallocating the dynamic memory
	*/
	void MakeEmpty();

	/*
	*	@brief	Expand the maximum size of the array.
	*/
	void ExpandSize();


	/*
	*	@brief	Reduce the maximum size of the array
	*/
	void ReduceSize();

	/*
	*	@brief	= operator for Deep copying
	*/
	SortedArrayList<T>& operator=(const SortedArrayList<T>& s_l);

private:
	T* m_List;					//List address pointer
	int m_maxsize;				//List maximum size
	int m_curPos;				//Current index position of array
	int m_length;				//Current length of array
};
