...
public:
	Queue();							//생성자
	int GetLength();					//학습단어 목록에서 남아있는 단어 갯수를 반환
	void enQueue(T& voca);				//학습단어 목록에 단어를 추가한다.
	void deQueue(T& voca);				//학습단어 목록에서 단어를 추출한다.
	bool isFull();						//학습단어 목록이 꽉 찼는지 검사한다.
	bool isEmpty();						//학습단어 목록이 비었는지 검사한다.
	void MakeEmpty();					//학습단어 목록을 비운다.
private:
	T m_LearnVocaList[maxQueue + 1];	//T형 데이터 타입을 요소로 갖는 크기 200의 배열
